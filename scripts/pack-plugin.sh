...
#
# 用法：
#   ./scripts/pack-plugin.sh <插件目录名> [-version x.y.z]
...
#   4. 产物：dist/{id}-{version}-{os}-{arch}.bpk（上传 GitHub Release 供市场安装）
set -euo pipefail

PLUGIN_DIR_NAME="${1:?用法: ./scripts/pack-plugin.sh <插件目录名> [-version x.y.z]}"
shift || true
VERSION_ARG=""
