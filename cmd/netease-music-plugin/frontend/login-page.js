...
export default function registerPage(ctx) {
  ...
  // 拉取播放地址并播放（验证 /song-url 链路）。
  const playSong = async (songId, btn) => {
    stopAudio();
    btn.textContent = "加载中…";
    btn.disabled = true;
    try {
      const r = await ctx.api.post("/song-url", { id: songId });
      btn.disabled = false;
      if (r.error) {
        btn.textContent = "▶ 试播";
        searchResult.querySelector("[data-play-msg]").textContent = r.error;
        return;
      }
      ...
      btn.disabled = false;
      btn.textContent = "▶ 试播";
      searchResult.querySelector("[data-play-msg]").textContent = "播放失败：" + String(e);
    }
  };
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      for (const s of songs) {
        ...
        btn.addEventListener("click", () => playSong(s.id, btn));
