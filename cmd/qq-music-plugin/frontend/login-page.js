...
export default function registerPage(ctx) {
  ...
  const S = {
    card: "border-radius:12px;border:1px solid var(--yy-border,#2a3348);background:var(--yy-elevated,#fff)",
    hint: "font-size:12px;color:var(--yy-text-2,#9aa6bc)",
  };
  ...
  // 骨架：标题 + Tab 导航 + 三个面板（登录/歌单/设置）
  box.innerHTML =
    '<div style="display:flex;align-items:center;gap:12px">' +
    '<span style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;background:' + GREEN + ';color:#fff;font-size:19px;font-weight:700">Q</span>' +
    '<div><h1 style="font-size:18px;font-weight:700;color:var(--yy-text,#e8ecf4);line-height:1.3">QQ 音乐</h1>' +
    '<p style="' + S.hint + '">扫码登录 · 歌单管理 · 首页背景音乐</p></div></div>' +
    '<div style="margin-top:16px;display:flex;gap:8px;border-bottom:1px solid var(--yy-border,#2a3348)">' +
    '<button type="button" data-tab="login" style="height:36px;padding:0 16px;font-size:13px;font-weight:600;color:#fff;background:transparent;border:none;border-bottom:2px solid ' + GREEN + ';cursor:pointer">QQ音乐登录</button>' +
    '<button type="button" data-tab="playlists" style="height:36px;padding:0 16px;font-size:13px;font-weight:600;color:var(--yy-text-2,#9aa6bc);background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer">我的歌单</button>' +
    '<button type="button" data-tab="settings" style="height:36px;padding:0 16px;font-size:13px;font-weight:600;color:var(--yy-text-2,#9aa6bc);background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer">设置</button>' +
    '</div>' +
    '<div style="margin-top:16px" data-panel-login></div>' +
    '<div style="margin-top:16px;display:none" data-panel-playlists></div>' +
    '<div style="margin-top:16px;display:none" data-panel-settings></div>';

  const panelLogin = box.querySelector("[data-panel-login]");
  const panelPlaylists = box.querySelector("[data-panel-playlists]");
  const panelSettings = box.querySelector("[data-panel-settings]");
  let audio = null;
  let pollTimer = 0;
  ...
  // showTab 切换 Tab（高亮 + 面板显隐）。
  const showTab = (name) => {
    const map = { login: panelLogin, playlists: panelPlaylists, settings: panelSettings };
    for (const [k, panel] of Object.entries(map)) panel.style.display = k === name ? "block" : "none";
    box.querySelectorAll("[data-tab]").forEach((b) => {
      const active = b.dataset.tab === name;
      b.style.color = active ? "#fff" : "var(--yy-text-2,#9aa6bc)";
      b.style.borderBottomColor = active ? GREEN : "transparent";
    });
  };

  // playSong 播放/停止切换（songmid 歌曲 MID；btn 触发按钮；msgEl 可选错误提示元素）。
  const playSong = async (songmid, btn, msgEl) => {
    if (audio && !audio.paused) { audio.pause(); audio = null; btn.textContent = "▶"; return; }
    stopAudio();
    btn.textContent = "…";
    btn.disabled = true;
    try {
      const r = await ctx.api.post("/song-url", { songmid });
      btn.disabled = false;
      if (r.error) { btn.textContent = "▶"; if (msgEl) msgEl.textContent = r.error; return; }
      audio = new Audio(r.url);
      audio.play().catch(() => {});
      audio.onended = () => { audio = null; btn.textContent = "▶"; };
      btn.textContent = "⏸";
    } catch (e) {
      btn.disabled = false;
      btn.textContent = "▶";
  ...
  const loadSongs = async (tid, wrap) => {
    ...
    try {
      ...
      for (const s of songs) {
        ...
        btn.addEventListener("click", () => playSong(s.song_mid, btn, null));
  ...
  return () => { stopAudio(); stopPoll(); box.remove(); };
}
